...
rf_gridsearch = train(form = card_accepted~family_size+spend_capacity_log1p+nbr_tot_cards+
                          ...
                          max_spent_ind_apparel_bool+pred_prob,
                      data=train_data[train_data$card_accepted != 'None',], method="rf", 
                      metric='Mean_Pos_Pred_Value', trControl=rf_ctrl,ntree = 1000,tuneGrid=tunegrid,sampsize = c(600,430,475))
print(rf_gridsearch)
print(rf_gridsearch$finalModel)

set.seed(3)
seeds = vector(mode = "list", length = 31)
for(i in 1:30) seeds[[i]] = sample.int(1000,7)
seeds[[31]] = sample.int(1000,1)
print(seeds)
rf_ctrl = trainControl(method = "repeatedcv",number = 10,repeats = 3,seeds = seeds,verboseIter = T,
                       allowParallel = T,search = 'grid',summaryFunction = twoClassSummary,classProbs = T)
set.seed(335)
tunegrid = expand.grid(.mtry = c(1,2,5,10,15,20,25))

rf_binary = train(form = accepted~family_size+spend_capacity_log1p+nbr_tot_cards+
                          nbr_mths_acct+mem_fee_24m+score_affinity_spend+income_log1p+
                          internal_influencer_score+elite_card_ind+score_affinity_business_spend+
                          cnt_payments_12m+spend_capacity_per_mem+
                          nbr_clb_mem+nbr_air_miles_mem+spend_ec_pct+spend_hotel_pct+
                      ...
                      metric='ROC', trControl=rf_ctrl,ntree = 1000,tuneGrid=tunegrid,sampsize = c(1000,1000))
print(rf_binary)
print(rf_binary$finalModel)


#plot(rf_gridsearch)
testing_data$rf_pred = predict(object = rf_gridsearch$finalModel,newdata = testing_data)
...
credit_roi = ((0.2505*9500))/1800;print(credit_roi)
elite_roi = ((0.2122*11000))/1500;print(elite_roi)
supp_roi = ((0.2620*8000))/1200;print(supp_roi)
...
predict_final = function(data_frame,logistic_model=log_reg_model$finalModel,rf_binary_model = rf_binary,lo,rf_model=rf_gridsearch){
    data_frame = create_new_vars(data_frame)
    data_frame$pred_prob = predict(object = logistic_model,newdata = data_frame,type = 'response')
    data_frame$rfb_pred_prob = predict(object = rf_binary_model,newdata = data_frame,type = 'prob')[[1]]
    ...
    data_frame$rf_prob_supp = predict(rf_model,newdata = data_frame,type = 'prob')[[3]]
    data_frame$rf_pred = predict(rf_model,newdata = data_frame)
    log_pred_prob = data_frame$rfb_pred_prob
    prob_threshold = 0
    while(prob_threshold <= 1){
        temp_labels = data_frame$rf_pred
        temp_labels[log_pred_prob <= prob_threshold] = 'None'
        z = table(temp_labels)
        elite_targeted = ifelse("Elite" %in% names(z),(z[['Elite']]),0)
        credit_targeted = ifelse("Credit" %in% names(z),(z[['Credit']]),0)
        supp_targeted = ifelse("Supp" %in% names(z),(z[['Supp']]),0)
        budget = credit_targeted*1800 + supp_targeted*1200 + elite_targeted*1500
        if(budget <= 1000000){break}
        prob_threshold = prob_threshold + 0.00001
    }
    print(prob_threshold);print(budget)
    print(paste('No. of Elite targeted:',elite_targeted))
    print(paste('No. of Supp targeted:',supp_targeted))
    print(paste('No. of Credit targeted:',credit_targeted))
    #Intersections:
    data_frame$card_offered = temp_labels
